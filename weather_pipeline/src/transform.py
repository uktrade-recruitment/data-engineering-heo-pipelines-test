...
def transform_weather_data(raw_data):
    ...
    transformed_data = []
    timestamp_formats = ['%Y-%m-%d %H:%M:%S', '%Y-%m-%dT%H:%M:%S', '%m/%d/%Y %H:%M:%S']
    ...
    for record in raw_data:
        try:
            ...
            transformed_record['station_name'] = record['station_name'].strip().upper() if 'station_name' in record and record['station_name'] else 'UNKNOWN'

            timestamp = None
            for fmt in timestamp_formats:
                try:
                    timestamp = datetime.strptime(record['timestamp'], fmt)
                    break
                except ValueError:
                    continue
            ...
            try:
                ...
                logging.warning(f"Invalid temperature value for record {record['station_id']}: {record['temperature']}")
                continue
            ...
            logging.error(f"Error transforming record {record.get('station_id', 'Unknown')}: {str(e)}")
            continue

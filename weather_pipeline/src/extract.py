...
def extract_weather_data(directory_path, date_str):
    ...
    """
    logging.info(f"extract weather data from {directory_path} for date {date_str}")

    try:
        all_data = []

        for filename in os.listdir(directory_path):
            if date_str in filename and filename.endswith('.csv'):
                ...
                logging.info(f"Extracted {len(all_data)} records from {filename}")
